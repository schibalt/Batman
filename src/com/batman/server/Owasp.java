...
public class Owasp {
	...
	try {
		...
		if (rs.next()) { 
		    ...
		    // Should not append, because login is the
				 // primary key
		    throw new SQLException(
		...
		// TIME RESISTANT ATTACK (Even if the user does not exist
		     // the
		     // Computation time is equal to the time needed for a
		     // legitimate user
	...
	return decoder.decode(byteString.toByteArray());
    }
	...
	byte[] bytes = endecoder.encode(data);
